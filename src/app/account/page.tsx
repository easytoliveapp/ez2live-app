...
import { type IDeleteUSer, type userLoginResponseProps } from '@/types/user'
...
const MyAccountPage = () => {
    ...
    const handleDeleteUser = async (values: IDeleteUSer) => {
      ...
      if (user != null) {
        await usersService.eraseUser(user.id, values.password)
          .then(() => {
            useToastify({ label: 'Conta excluída com sucesso', type: 'success' });
            router.push('/login');
          })
          .catch(() => {
    ...
    const handleSendEmailChangePassword = async () => {
      ...
      if (user && user.email) {
        await authService.forgotPassword({ email: user.email })
          .then(() => {
            useToastify({ label: 'Foi enviado um link para redefinir a senha ao endereço de email cadastrado.', type: 'success' });
      ...
      <div className='relative h-max flex flex-col mx-auto gap-4 w-full max-w-md'>
        <ModalEdit show={handleModal} onCloseModalEdit={() => { setHandleModal(false) }} >
          <div className='flex flex-col h-[85vh]'>
            ...
            <p className='my-4 text-black'>Ao apagar sua conta, todas as suas reservas e outros dados serão apagados <b>permanentemente</b> do nosso sistema. Se você tem certeza disso, digite a senha da sua conta.</p>
        ...
        <ButtonSecondary
          onClick={async () => { await handleSendEmailChangePassword() }}
          loading={loading}
  ...
  const TabComponent = useCallback(() => pageId === 'account' ? <Account /> : <Security />, [pageId])
