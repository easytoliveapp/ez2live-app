...
export const WaitingApprovalStep: React.FC<IWaitingApprovalStepProps> = ({
  ...
  useEffect(() => {
    if (rendered && invoiceStatus !== "paid") {
      setTimeout(() => getInvoiceStatus(), 5000);
    }
  }, [invoiceStatus, rendered]);

  const getInvoiceStatus = async () => {
    await subscriptionService
      .getInvoices()
      .then((res: any) => {
        setInvoiceStatus(res.data.items[0].status);
      })
      .catch(() => {
        showToastify({
              ...
              {PaymentTab === PAYMENT.pix && (
                <div>
                  <SimpleModal className="pb-6">
                    ...
                    <p className="font-bold text-xs text-generic-dark">
                      QR Code
                    </p>
                    <QRCode
                      value={qrCodeValue}
                  ...
                  <SimpleModal className="flex text-xs flex-col px-5 justify-start items-baseline">
                    ...
                    <ol className="list-decimal px-3">
                      <li>
                        Abra o site ou aplicativo do seu banco e selecione a
                        opção PIX
                      </li>
                      <li>Escolha pagar com QR code</li>
                      <li>Aponte sua câmera para o QR code exibido na tela</li>
                      <li>
                        Tudo certo! Confira os dados e finalize o pagamento
                      </li>
                    </ol>
                    <Image
                      alt="qr-code-simple-icon"
                    ...
                    <ol className="list-decimal px-3">
                      ...
                      <li>
                        Selecione a opção “Copia e cola” e insira o código
                      </li>
                      <li>
                        Tudo certo! Confira os dados e finalize o pagamento
                      </li>
